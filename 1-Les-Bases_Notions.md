...
4. [Écosystème et déploiement de Kubernetes](#4-écosystème-et-déploiement-de-kubernetes)
5. [Valeur et apports fonctionnels de Kubernetes](#5-valeur-et-apports-fonctionnels-de-kubernetes)
...
6. [Les `status`](#6-les-status)

V. [**ReplicaSets**](#v-replicasets)
1. [ReplicaSets : présentation](#1-replicasets--présentation)
2. [`labels` et `selector`](#2-labels-et-selector)

VI. [**Deployments**](#vi-deployments)
1. [Deployments : présentation](#1-deployments--présentation)
2. [Rollouts strategies](#2-rollouts-strategies)
...
- **Meilleurs performances** des déploiements : **déploiement à l'échelle** et **capacité d'itération** très importantes (ex. : le failfast, la capacité à itérer de plus en plus souvent). Facile de revenir sur des versions antérieures pour satisfaire le besoin des utilisateurs finaux.
...
- Améliorations Opensource, segmente le développement : permet d'embarquer plus de monde sur des petites tâches plutôt que sur un gros projet. Permet aussi la mise en place à grande échelle de projet opensource.
...
- On installe sur un hôte, au-dessus du système d'exploitation un **Container Engine**, il va utiliser les **cgroups** et les **namespaces** dans le but de lancer un processus (ou un groupe de processus) afin qu'il "croie" qu'il est isolé (son propre réseau, hostname, user ...etc.). Il ne verra pas les autre processus qui fonctionnent en parallèle.
- Pas d'émulation de ressources, pas d'OS dédié, moins de maintenance tout en étant isolé.

### **Image** vs **Container**

- **Image** = **le livrable** (le fichier tar.gz avec ses metadata, filesystème, processus lancé et ses dépendances), elle pourra être stockée dans des Registries.
- **Container** = un **processus actif d'une image donnée**. L'image pourra être instanciée (avoir plusieurs instance de conteneurs actifs de l'image)
...
- **Docker Engine** : est une plateforme de gestion de conteneurs (daemon), il va ajouter des fonctionnalités au-dessus de Containerd pour faciliter son fonctionnement. Management facile des images, containers, réseau et volumes.
...
- **Besoins** :
	...
	- **Autohealing**, facilitation de management des process, avoir un **Control Plane** (une vision d'ensemble) pour manager les process.
...
## 4. Écosystème et déploiement de Kubernetes
...
- Un **Installer** pour déployer un cluster Kubernetes. Ex. : *kubeadm*, *kubespray* (installation ansible de Kubernetes avec des configuration plus poussées comme des volumes dynamiques, Ingress Controller...)
- Version de Kubernetes **allégées**, comme k3s (un Kubernetes très épuré), k3d, minikube,microk8s, k0s.

## 5. Valeur et apports fonctionnels de Kubernetes

- **Améliore les déploiements** (méthode GitOps, il pourra être installé au sein du cluster)
- Implémentation de la **haute disponibilité (HA)** au niveau de l'**infrastructure** et des **applicatifs**. Le cluster Kubernetes gère les deux sans intervention humaine. De la même manière il intègre de l'**auto guérison**.
...
Le changement de paradigme (impératif vers déclaratif) est au cœur de l'architecture de Kubernetes, tout passe par l'API de Kubernetes. C'est le point d'entrée pour le système déclaratif qui permet de faire fonctionner le cœur de Kubernetes (l'orchestration).
...
- L'action **doit être managée, configurée et exécutée** par le demandeur.
...
- **Moins de contrôle** sur les actions réalisées, mais **plus d'actions déléguées** à d'autres personnes.
- Analogie : On **décrit ce que l'on "veut"** installer.

## 2. Master et Worker

Ils assurent et assument la fonction déclarative mise à disposition par Kubernetes. Le mode déclaratif va gérer l'ensemble des états possible et tenter d'assumer et garantir l'état déclaré.
...
- Il reçoit les instructions par une API et les stocke dans une base de données.
 ...
 **Worker** :
- Ils peuvent être configurés avec des profils différents (ex. avec GPU, sans GPU, avec ARM...etc.). Permettent d'**adapter/manager les workloads** installés sur les Worker.
- Ils fournissent les **capacités de calcul**, la **RAM**, **GPU** afin d'économiser les ressources.
...
`kubectl` est un client qui communique de manière **sécurisée** (certificats, autorisation, authentification) avec une API. Le but étant d'envoyer des requêtes à l'API-server (API en REST). Ces requêtes peuvent aussi être envoyées par des **librairies** (python, Go, etc.) ou du `curl`.

Typiquement on retrouve nos **2 types** de serveurs :

### **Node Master** :

- **kube-api-server** : Premier point d'entrée dans le cluster Kubernetes. Située dans le Control Plane (**Rappel** : le Control Plane est **l’ensemble logique des composants** Kubernetes qui pilotent l’état du cluster.) 
- **etcd** : La **base de données** qui permet de **stocker les informations** du cluster (les manifestes, l'état du cluster, ses noeud/spécificités, les application déployées, etc.)
...
- Les informations stockées dans la base de données *etcd* représentent l'**état du cluster dans son ensemble** (l'état du serveur, les configurations, l'état des Pods, les secrets, etc.).
...
- Cette base de données **clé/valeur** repose sur l'**algorithme raft**, qui met en place un système d’élection par vote **basé sur une majorité**. Il est donc recommandé d'utiliser un nombre **impair** de nœuds afin d'éviter les blocages et permettre d'élire un **leader**.
...
- En tant que base de données il est préférable de faire des backups de l'*etcd*.

>[!NOTE]
>Backup direct de la base de données *etcd* avec les lignes de commandes `etcdctl` ou avec des outils dédiés à la sauvegarde et à leur management.
...
- Le contrôleur est une fonction qui prend un **état désiré**, le compare à un **état réel** et en **conclue des actions à effectuer**. Chaque contrôleur fait ce travail sur un composant logique donné au niveau du cluster (ex. : un pour les Deployments, un pour les noeuds, un pour les ReplicaSets, etc.).
...
>[!IMPORTANT]
> Plus les informations fournies au *scheduler* et au *contrôleurs de déploiement* sont exactes, plus l'allocation et les actions menées seront précises et correctes.
...
- Le *kubelet* **remonte des informations** à l'*APIServer* pour le tenir informé afin qu'il en déduise des actions éventuelles.

---
<a id="iii-lapiserver"></a>
...
- C'est une API en **REST** (REpresentational State Transfer), 
...
- Elle fait du **RBAC** (Role Based Access Control, association entre l'utilisateur et le rôle) en gérant deux niveaux de sécurité : 
	...
	- l'**autorisation** (est ce que la personne authentifiée a les droits pour réaliser les actions désirées).
...
>[!NOTE]
>Il y a plusieurs niveaux de versions :
>- `v1alpha<x>` : versions expérimentales, modifiables par la communauté et développeurs.
>- `v1beta<x>` : version plus stable, activée par défaut dans le cluster.
>- `v<x>` : version stable, rétrocompatibilités assurées.

Il est possible de créer ses propres groupes quand on crée ses propres ressources avec les **CRD** (Custom Resource Definition) :
```yaml
...
Si, on veut avoir accès à plusieurs cluster Kubernetes, on peut éditer la variable `$KUBECONFIG`, mais aussi modifier le fichier en y ajoutant les nouveaux clusters
...
Le pod est l'objet logique central de Kubernetes, il permet de manager des services et des applicatifs au niveau du cluster.
## 1. Rappel sur le pod

- C'est un objet logique qui permet de regrouper un ou plusieurs conteneurs dans un même **contexte**.
	- Il existe **différents types** de conteneurs qui vont être lancés en séquence (des container lancés à l'init du pod, d'autres à la fin, et des container qui vont fonctionner tout au long de la vie du pod).
...
- Le Pod est l'objet le plus important pour les opérations de *scaling* (mise à l'échelle). 
...
- Les deux conteneurs sont dans les mêmes *namespaces*. Le *network namespace* pourra être partagé/mutualisé par le container 1 et le container 2, ils utilisent donc le même réseau. 
...
spec: # les spécifications de ce déploiement
  strategy:
    type: RollingUpdate
    rollingUpdate:
      maxSurge: 1
  ...
  selector: # Permet de choisir/filtrer les pods à associer au réplicats
...
## [**Mise en pratique : création d'un cluster k0s**](1-Les-Bases_Pratiques.md#i-mise-en-place-dun-cluster-k0s)
## [**Mise en pratique de `kubectl` & `curl`**](1-Les-Bases_Pratiques.md#ii-kubectl--curl)
## [**Lancement des premiers Pods**](1-Les-Bases_Pratiques.md#iii-lancement-des-premiers-pods)
...
>[!NOTE]
>Même s'il existe trois patterns qui ont leurs propre rôles, ils sont communément regroupés sous l'étiquette **sidecar**.

## [**Pods : Multi-Conteneurs**](1-Les-Bases_Pratiques.md#iv-pods--multi-conteneurs)

## 6. Les `Status`

### 6.1 Contexte 

- Le champ **`status`** représente l’**état observé** d’un objet Kubernetes (pod, node, etc.).
- Il est **géré automatiquement par Kubernetes** et ne doit pas être modifié manuellement.
- Le contenu du `status` dépend du type de ressource.

**Dans le cas du Pod** :
Pour un **Pod**, le `status` se compose principalement de :
-  La **phase** : une agrégation d’informations observées (scheduling, état des conteneurs) fournissant une vue synthétique de son état global.
	- Exemples : `Pending`, `Running`, `Succeeded`, `Failed`, `Unknown`
- Les **conditions** : Une liste d’états booléens décrivant des aspects précis du pod
	- Exemples : `PodScheduled`, `Ready`, `ContainersReady`

Les informations de `status` sont récupérées depuis :
- Le **scheduler** qui met à jour la condition `PodScheduled`
- Le **kubelet** qui met à jour l'état des conteneurs ainsi que les conditions liées à l’exécution et à la disponibilité du pod.
Les informations du `status` reflètent l’état réel observé dans le cluster.

>[!IMPORTANT]
>La **`phase`** d’un Pod est une **synthèse** calculée à partir de plusieurs éléments, notamment :
>- l’état du scheduling (le pod est-il assigné à un node ?)
>- l’état des conteneurs :
>    - démarrés ou non
>    - en cours d’exécution
>    - terminés avec succès ou en erreur
>- la capacité du kubelet à rapporter l’état du pod
>
>Ces informations proviennent principalement du **kubelet**.
>- Le **kubelet** observe l’état réel des conteneurs sur le node
>- Il remonte ces informations à l’API Server
>- Kubernetes **déduit la `phase`** à partir de cet état observé

Illustration des `type` `phase` dans le code source de Kubernetes :
https://github.com/kubernetes/kubernetes/blob/v1.31.1/pkg/apis/core/types.go
```go
...
Illustration des `type` `condition` dans le code source de Kubernetes :
https://github.com/kubernetes/kubernetes/blob/v1.31.1/pkg/apis/core/types.go
```go
...
### 6.2 Contenu d'un `status`

Un `status` contient plusieurs éléments :
- La **`phase`** (`pod.status.phase`), la vue synthétique de l’état global du pod.
- La **`reason`** (`pod.status.reason`), optionnelle, elle est utilisée quand le pod est en `Failed` ou `Pending`. Ainsi, elle indique la cause d’un état particulier (ex. échec, blocage).
- Les **`conditions`** (`pod.status.conditions`), les états booléens décrivant des aspects précis du pod (Ready, PodScheduled, etc.).
- Le **`status` des conteneurs d'initialisation** (`pod.status.initContainerStatuses`)
- Le **`status` des conteneurs applicatifs** (`pod.status.containerStatuses`).

Les sections suivantes décrivent soit des **phases de pod**, soit des **états de conteneur**, tels qu’observés via `kubectl`.

>[!TIP]
>La commande `kubectl explain` permet d'avoir des informations sur les différents champs du manifeste d'une ressource (pod, ReplicaSet, etc.).
>_Exemples_ :
>```bash
>kubectl explain pod
...
### 6.3 `ContainerCreating`

- C'est un **état transitoire normal d’un conteneur**.
- Il indique que le conteneur est en cours de préparation
- Le `container-runtime`, piloté par le **kubelet**, effectue notamment :
	- le téléchargement de l’image
	- la création du conteneur
	- la configuration de l’environnement (volumes, réseau, etc.)
- Cet état **ne correspond pas à une erreur**, mais il peut durer plus longtemps en cas de :
	- lenteur réseau (pull de l’image)
	- attente de volumes
	- contraintes sur le node

### 6.4 `Running`

- C'est un **état normal attendu pour conteneur** en cours d’exécution.
- Tous les conteneurs applicatifs **long-running** sont généralement dans l’état `Running`.

### 6.5 `Completed`

- Ce n'est pas une erreur, cet état indique qu’un **conteneur** a **terminé son exécution avec succès** (code de sortie 0).
- Il ne correspond pas forcément à l'état souhaité. Pour le débugger :
	- `kubectl logs <nomDuPod>`
	- `kubectl describe pods`

### 6.6 `Terminating` 

- Ce n'est pas une erreur, c’est un **état d’affichage** qui indique qu'un pod est en train de s'arrêter.
- Cet état peut être long ou bloqué :
	- Causé par un `finalizer`, un paramètre mis en place qui permet de bloquer la suppression tant qu’une action n’est pas terminée.
	- Causé par une `grace period`, le délai accordé aux conteneurs d’un pod pour s’arrêter proprement avant leur arrêt forcé.
- Si l'état reste bloqué sur `Terminating` quand on veut arrêter un pod, son arrêt peut être forcé:
	- `kubectl delete pods <nomDuPod> --force --grace-period=0`

>[!NOTE]
>Détail de l'**état** `Terminating` :
>1. Kubernetes marque le pod pour suppression
>2. Le **kubelet** envoie un **SIGTERM** aux conteneurs
>3. Les conteneurs ont un délai pour s’arrêter proprement (`terminationGracePeriodSeconds`, 30s par défaut)
>4. Passé ce délai, un **SIGKILL** peut être envoyé
>5. Le pod est finalement supprimé
### 6.7 `Pending`

- Ce n'est pas une erreur.
- C'est une **phase** qui indique que le pod n’est pas encore prêt à s’exécuter (scheduling, pull d’image, init containers, etc.).
	- Causé par une absence de ressources : pas de volumes, pas assez de CPU/RAM, etc.
- Pour débugger cette **phase** :
	- `kubectl describe pod <nomDuPod>`
	- `kubectl get events`
- C'est dans cet état que les décisions sont prises pour étendre le cluster dans le contexte de l'*autoscaling* (ajout de node worker automatique).

### 6.8 `CrashLoopBackOff`

- C'est une erreur réelle, dans cet **état** Kubernetes redémarre le pod plusieurs fois.
	- Les temps entre les redémarrages ne sont pas linéaires mais exponentiels (ex. : redémarrage au bout de 10s, puis 20s, puis 40s, etc.).
- On peut monitorer cet état depuis la commande `kubectl get pods <nomDuPod>` en observant la colonne `READY = 0/x`, et ainsi voir le nombre de redémarrage.
- Cet état peut être causé par des erreurs de code, de configurations, de permission ou de limites.
- Pour débugger, cet **état** :
	- `kubectl logs <nomDuPod>`

### 6.9 `Failed`

- C'est une **phase** d'un pod.
- Tous les conteneurs ont terminé en erreur et ne seront pas redémarrés.

### 6.10 `ImagePullBackOff` / `ErrImagePull`

- C'est une erreur réelle, l'image depuis le registry ne peut pas être pull.
- Fréquemment causé par un tag d'image, un nom d'image, ou encore l'URL pour accéder au répertoire mal définis.
- Pour débugger, cet **état** :
	- `kubectl describe pods <nomDuPod>`

### 6.11 `Evicted`

- C'est une **raison** qui indique que le node subit trop de pression par contrainte de ressources. Il y a un besoin de libérer de la ressource.
- Le pod passe ensuite en `Failed`
- Pour débugger, cette **raison** :
	- `kubectl describe nodes <nomDuNode>`
	- `kubectl get events`

### 6.12 `OOMKilled`

- Cet **état** indique qu’un conteneur a dépassé sa limite mémoire et a été tué par le kernel.
- Pour débugger, cet **état** :
	- `kubectl top pods`
	- `kubectl get pods`
	- `kubectl describe pods`
	- Du monitoring sur la mémoire.

---
<a id="v-replicasets"></a>
# V. [**ReplicaSets**](#index)

## 1. ReplicaSets : présentation

- Le Replicaset est un objet Kubernetes géré par le **`replicaset controller`** (**`replication controller`** - legacy). Ce contrôleur surveille les objets de type ReplicaSets présents dans l'API Server afin de **créer ou supprimer des Pods** en fonction de l’état désiré.
- Le ReplicaSet permet de **gérer le nombre de Pods** et de **s’assurer que le nombre de Pods désiré correspond au nombre de Pods réellement en cours d’exécution**.
- En pratique, il est rarement manipulé directement : on privilégie l’utilisation des **Deployments**.
	- Il fournit ses fonctionnalités au **Deployment**
	- En effet le Replicasets, à lui seul, ne permet pas de faire de l'autoscaling.

## 2. `labels` et `selector`

Le **ReplicaSet ne référence pas directement des Pods existants dans son manifeste**.  
Il définit un **template de Pod** (`Pod.Template.Spec`) qui sera utilisé pour créer les Pods nécessaires.
Afin de pouvoir identifier et différencier les Pods qu’il doit gérer, le ReplicaSet s’appuie sur un mécanisme de **labels** et de **selectors**.

#### Les `Labels`

- Les **labels** sont des paires clé/valeur définies sur les Pods. Ils permettent d’identifier et de regrouper les ressources Kubernetes.
...
- Le **selector** permet au ReplicaSet de déterminer quels Pods il doit gérer, en se basant sur leurs labels.
...
Les **labels définis dans le Pod template doivent correspondre au selector du ReplicaSet**.  
Cette correspondance permet au ReplicaSet d’identifier les Pods qu’il doit créer et maintenir.
Ainsi, si un Pod possède les labels correspondant au selector :
- il est **automatiquement pris en compte**
- même s’il n’a pas été créé par ce ReplicaSet

>[!IMPORTANT]
>Le selector d’un ReplicaSet est **immuable**.
>Une mauvaise définition des labels peut entraîner la prise en charge de Pods non désirés.

_Exemple de manifeste d'un ReplicaSet_ :

![exempleManifestReplicaSet](./img/exempleManifestReplicaSet.png)

## [**Exemples sur les ReplicaSets**](1-Les-Bases_Pratiques.md#v-replicasets)

---
<a id="vi-deployments"></a>
# VI. [**Deployments**](#index)

## 1. Deployments : présentation

- Le **Deployment** est un objet qui permet de gérer le déploiement d’une application de façon déclarative.
-  Il conserve un historique des différentes versions et permet de revenir facilement à une version précédente.
- Il permet aussi de définir des stratégies de déploiement afin de contrôler précisément les mises à jour.
- Un Deployment **ne crée pas directement les Pods** : il gère des **ReplicaSets**, qui eux-mêmes gèrent les Pods.
- Le Deployment s’assure de **maintenir l'état désiré** :
	- Le nombre de Pods souhaité est toujours respecté
	- Les Pods sont recréés automatiquement en cas de panne
- Permet des **mises à jour progressives (rolling update)** :
	- Mise à jour sans interruption de service
	- Contrôle du nombre de Pods indisponibles

_Exemple d'un manifest de Deployment_ :
![exempleManifestDeployment](./img/exempleManifestDeployment.png)

**Tableau comparant les fonctionnalités des ReplicaSets vs Deployments** :

| **Feature/Setting**    | **ReplicaSet** | **Deployment**              |
| ---------------------- | -------------- | --------------------------- |
| **Replicas**           | ✅              | ✅                           |
| **Selector**           | ✅              | ✅                           |
| **Pod Template**       | ✅              | ✅                           |
...
| **Scale Management**   | Basic Scaling  | Declarative Scaling         |
| **Pausing/Resuming**   | ❌              | ✅                           |
| **Progress Deadline**  | ❌              | ✅                           |
| **MinReadySeconds**    | ❌              | ✅                           |

>[!NOTE]
>Kubernetes supporte principalement deux stratégies de déploiement : **RollingUpdate** et **Recreate**.

## 2. Rollouts strategies

Une _rollout strategy_ définit la manière dont Kubernetes met à jour les Pods d’un `Deployment` lorsqu’une modification est appliquée (par exemple une nouvelle image Docker).

Kubernetes propose principalement **deux stratégies** : `Recreate` et `RollingUpdate`.

### 2.1 `Recreate`

- La stratégie `Recreate` consiste à **supprimer tous les Pods existants**, puis à **créer les nouveaux Pods** avec la nouvelle version.
- Pendant la mise à jour, **aucun Pod n’est disponible**, ce qui entraîne une **interruption de service (downtime)**.

>[!NOTE]
>Cette stratégie est **peu utilisée en production**, mais peut être pertinente dans certains cas :
>- Applications **stateful** ou ne supportant pas plusieurs versions en parallèle ;
>- Contraintes fortes sur les ressources ;
>- Environnement de test ou de développement.

**Exemple de comportement** :
1. Tous les Pods de la version actuelle sont arrêtés ;
2. Les nouveaux Pods sont créés avec la version mise à **jour**.

### 2.2 `RollingUpdate`

 La stratégie `RollingUpdate`  est la stratégie **par défaut** utilisée par Kubernetes.
- Elle permet de **mettre à jour les Pods progressivement**, sans interruption de service.
- Les anciens Pods sont remplacés par des nouveaux **par petits groupes**, ce qui garantit une **haute disponibilité**.

Elle repose sur deux paramètres clés :
- `maxUnavailable`  
    - Le nombre maximum de Pods **indisponibles** pendant la mise à jour
- `maxSurge`  
	- Le nombre maximum de Pods **supplémentaires** pouvant être créés temporairement

>[!NOTE]
>Ces valeurs peuvent être exprimées :
>- en **nombre absolu** (`1`, `2`, ...)
>- en **pourcentage** (`25%`, `50%`, ...)

**Exemple de comportement** :
1. Kubernetes crée un nouveau Pod ;
2. Vérifie qu’il est prêt (`readinessProbe`) ;
3. Supprime ensuite un ancien Pod ;
4. Répète l’opération jusqu’à la fin du déploiement.

### 2.3 Paramètres de contrôle du rollout

#### `progressDeadlineSeconds`:

- `progressDeadlineSeconds` définit le **temps maximum autorisé** pour qu’un Deployment progresse correctement.
- Si ce délai est dépassé, le Deployment est marqué comme **`ProgressDeadlineExceeded`**.
- Cela permet de détecter un déploiement bloqué ou encore signaler une image cassée, une `readinessProbe` qui échoue, etc.

>[!IMPORTANT]
>La valeur de temps **doit** être plus grande que la valeur du `minReadySeconds`.

#### `minReadySeconds` :

- `minReadySeconds` définit le **temps minimum (en secondes)** pendant lequel un Pod doit être **Ready** avant d’être considéré comme **disponible**.
- Cela permet d’éviter qu’un Pod instable soit utilisé trop rapidement lors d’un déploiement.
- Kubernetes attend ce délai avant de supprimer un ancien Pod.
- Par défaut cette valeur est à 0.

_Exemple d'application du `minReadySeconds`_ :
- l’application démarre vite mais **met du temps à être réellement stable**
- éviter un enchaînement trop rapide création/suppression de Pods

>[!CAUTION]
>Ces paramètres ne sont pas des stratégies, il sont décrits dans le manifeste de Deployment dans le champ `spec` :
>```yaml
>...
>spec:
>  minReadySeconds: 10
...
## [**Exemples sur les Deployments**](1-Les-Bases_Pratiques.md#vii-deployments)
